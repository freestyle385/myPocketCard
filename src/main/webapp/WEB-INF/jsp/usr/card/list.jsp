...
<section id="list-wrap" class="cell">
  ...
  <form action="/usr/card/list" method="GET" id="search-form">     
    ...
    <div id="filter-wrap" class="row">
      ...
      <div class="filter-box cell-r">
        ...
        <div class="filter-select">
          <select name="learningStatus">
            <option value="-1" ${searchedLearn == -1 ? 'selected' : ''}>전체 선택</option>
            <option value="0" ${searchedLearn == 0 ? 'selected' : ''}>학습 필요</option>
            <option value="1" ${searchedLearn == 1 ? 'selected' : ''}>학습 완료</option>
          </select>
        </div>
      </div>
  	</div>
      ...
      <input name="searchKeyword" value="${searchedKeyword}" placeholder="질문과 답변의 키워드를 검색해보세요."/>
